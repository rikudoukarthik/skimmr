...
#'
#' @param dataset_str character; the string in the file name of the data download
...
unzip_ebd <- function(dataset_str = "ebd_IN_unv_smp_rel", unzip_which = c("ebd", "sed"),
  ...
  if (!exists("currel_month_lab") & !exists("currel_year")) {
  ...
  path_zip <- if (!is.null(dir_name))
    glue::glue("{dataset_str}/{dataset_str}{currel_month_lab}-{currel_year}.zip") else
  ...
  if (file_exist_check == TRUE) {
    ...
    print("Specified data already unzipped.")

  } else {

    if (!file.exists(path_zip)) {
      print("Specified zip file of data does not exist!")
    } else {
      unzip(zipfile = path_zip, files = list_files,
